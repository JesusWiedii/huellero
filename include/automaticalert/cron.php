...
if ($result) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($num == 0) {
            if (!empty($diff_entry) && '00:00:00' <= $diff_entry && $diff_entry <= '00:15:00') {
                $correo = 'si';
            ...
            if (empty($t_out)) {
                if (!empty($diff_departure) && '00:00:00' >= $diff_departure && $diff_departure >= '-00:05:00') {
                    $correo = 'si';
                    $bodymesage = 'No se le olvide registrar la huella al salir'
                        . date("d/m/Y");
                } elseif (!empty($diff_e_lunch) && '00:00:00' >= $diff_e_lunch && $diff_e_lunch >= '-00:05:00') { }
            }
        }
    }
} else {
